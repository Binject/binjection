...
func Test_Elf_Inject_Static_Nop_1(t *testing.T) {
	...
	os.Mkdir("tmp", 0755)
	err := Binject("test/static_ls", "tmp/static_ls_injected", "test/nop.bin", &BinjectConfig{CodeCaveMode: false, InjectionMethod: SilvioInject})
	if err != nil {
		t.Error(err)
	}
...
func Test_Elf_Inject_Exec_Hello_1(t *testing.T) {
	...
	os.Mkdir("tmp", 0755)
	err := Binject("test/static_ls", "tmp/static_ls_injected", "test/hello.bin", &BinjectConfig{CodeCaveMode: false, InjectionMethod: SilvioInject})
	if err != nil {
		t.Error(err)
	}
...
func Test_Elf_Inject_Exec_PTNOTE_Hello_1(t *testing.T) {
	...
	os.Mkdir("tmp", 0755)
	err := Binject("test/static_ls", "tmp/ls_ptnote_hallo.injected", "test/hallo.bin", &BinjectConfig{CodeCaveMode: false, InjectionMethod: PtNoteInject})
	if err != nil {
		t.Error(err)
	}
